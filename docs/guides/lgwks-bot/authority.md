...
(`crates/lgwks-bot/src/ecs.rs:3922`):
...
the world (`crates/lgwks-bot/src/ecs.rs:3939`):
...
- `EcsBot::assemble` admits every declared requirement before the world is built
  (`crates/lgwks-bot/src/ecs.rs:3923`), so a bot whose declared requirements are
...
that folds it into an abandoned entry (`crates/lgwks-bot/src/ecs.rs:2336`) is

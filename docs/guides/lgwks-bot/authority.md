...
(`crates/lgwks-bot/src/ecs.rs:4357`):
...
the world (`crates/lgwks-bot/src/ecs.rs:4374`):
...
- `EcsBot::assemble` admits every declared requirement before the world is built
  (`crates/lgwks-bot/src/ecs.rs:4357`), so a bot whose declared requirements are
...
that folds it into an abandoned entry (`crates/lgwks-bot/src/ecs.rs:2772`) is

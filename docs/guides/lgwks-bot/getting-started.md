...
(`crates/lgwks-bot/src/ecs.rs:4565`), because the condition on this executor *is*
...
(`crates/lgwks-bot/src/ecs.rs:3946`), so the effects fire in the order you wrote
the `.on` calls.
...
totally ordered (`crates/lgwks-bot/src/ecs.rs:3569`).
...
(`crates/lgwks-bot/src/ecs.rs:3624`):
...
tick `observe_fold` treats it as changed (`crates/lgwks-bot/src/ecs.rs:3230`), so

...
synchronous adapter (`crates/lgwks-bot/src/ecs.rs:3633`) and `Bot::tick_async` is
the one to `await` from inside a runtime (`crates/lgwks-bot/src/ecs.rs:3530`).
...
(`crates/lgwks-bot/src/ecs.rs:2943`):
...
(`crates/lgwks-bot/src/ecs.rs:3311`); the `run_steps` pass then awaits those steps
on the caller's executor in exactly that order
(`crates/lgwks-bot/src/ecs.rs:3787`). It breaks on the first failure and records
...
`fire_plan` decides (`crates/lgwks-bot/src/ecs.rs:3088`), so selecting work by
...
- A condition whose `Evaluate<T>` implementation returns `Err` stops the chain
  with that error (`crates/lgwks-bot/src/ecs.rs:3308`). A structural failure in a
...
(`crates/lgwks-bot/src/ecs.rs:1036`); `tick` takes it after the schedule runs, and

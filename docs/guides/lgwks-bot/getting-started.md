...
(`crates/lgwks-bot/src/ecs.rs:4614`), because the condition on this executor *is*
...
(`crates/lgwks-bot/src/ecs.rs:3995`), so the effects fire in the order you wrote
the `.on` calls.
...
totally ordered (`crates/lgwks-bot/src/ecs.rs:3618`).
...
(`crates/lgwks-bot/src/ecs.rs:3673`):
...
tick `observe_fold` treats it as changed (`crates/lgwks-bot/src/ecs.rs:3279`), so

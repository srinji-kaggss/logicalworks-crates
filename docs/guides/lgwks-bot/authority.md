...
(`crates/lgwks-bot/src/ecs.rs:4691`):
...
the world (`crates/lgwks-bot/src/ecs.rs:4708`):
...
- `EcsBot::assemble` admits every declared requirement before the world is built
  (`crates/lgwks-bot/src/ecs.rs:4691`), so a bot whose declared requirements are
  not granted does not exist to run.
- Every per-call proof is minted from **the same list**. `run_any` calls
  `grants.issue(self.0.required_caps())` (`crates/lgwks-bot/src/spec.rs:621`),
...
that folds it into an abandoned entry (`crates/lgwks-bot/src/ecs.rs:3088`) is

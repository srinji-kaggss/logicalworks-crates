...
(`crates/lgwks-bot/src/ecs.rs:4466`), because the condition on this executor *is*
...
(`crates/lgwks-bot/src/ecs.rs:3854`), so the effects fire in the order you wrote
the `.on` calls.
...
totally ordered (`crates/lgwks-bot/src/ecs.rs:3477`).
...
(`crates/lgwks-bot/src/ecs.rs:3532`):
...
tick `observe_fold` treats it as changed (`crates/lgwks-bot/src/ecs.rs:3138`), so

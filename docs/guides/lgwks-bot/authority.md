...
(`crates/lgwks-bot/src/ecs.rs:4118`):
...
the world (`crates/lgwks-bot/src/ecs.rs:4135`):
...
- `EcsBot::assemble` admits every declared requirement before the world is built
  (`crates/lgwks-bot/src/ecs.rs:4119`), so a bot whose declared requirements are
...
that folds it into an abandoned entry (`crates/lgwks-bot/src/ecs.rs:2477`) is

...
(`crates/lgwks-bot/src/ecs.rs:4539`), because the condition on this executor *is*
...
(`crates/lgwks-bot/src/ecs.rs:3927`), so the effects fire in the order you wrote
the `.on` calls.
...
totally ordered (`crates/lgwks-bot/src/ecs.rs:3550`).
...
(`crates/lgwks-bot/src/ecs.rs:3605`):
...
tick `observe_fold` treats it as changed (`crates/lgwks-bot/src/ecs.rs:3211`), so

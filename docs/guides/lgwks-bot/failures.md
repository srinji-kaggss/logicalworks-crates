...
synchronous adapter (`crates/lgwks-bot/src/ecs.rs:3891`) and `Bot::tick_async` is
the one to `await` from inside a runtime (`crates/lgwks-bot/src/ecs.rs:3788`).
...
(`crates/lgwks-bot/src/ecs.rs:3201`):
...
(`crates/lgwks-bot/src/ecs.rs:3346`); the `run_steps` pass then awaits those steps
on the caller's executor in exactly that order
(`crates/lgwks-bot/src/ecs.rs:3995`). It breaks on the first failure and records
...
`fire_plan` decides (`crates/lgwks-bot/src/ecs.rs:3346`), so selecting work by
...
(`crates/lgwks-bot/src/error.rs:125`). Its `Display` renders the domain, the
...
- A condition whose `Evaluate<T>` implementation returns `Err` stops the chain
  with that error (`crates/lgwks-bot/src/ecs.rs:3566`). A structural failure in a
...
(`crates/lgwks-bot/src/ecs.rs:1234`); `tick` takes it after the schedule runs, and

...
`Execute::execute_action` takes `(Auth, &Self::Input)`. `Bot::tick` awaits it in
declaration order, so the effects fire in the order you wrote the `.on` calls.
...
tick `observe` treats it as changed (`crates/lgwks-bot/src/ecs.rs:257`), so the
condition is evaluated on the first poll even if the value never moves again.

...
synchronous adapter (`crates/lgwks-bot/src/ecs.rs:2929`) and `Bot::tick_async` is
the one to `await` from inside a runtime (`crates/lgwks-bot/src/ecs.rs:2821`).
...
(`crates/lgwks-bot/src/ecs.rs:2261`):
...
(`crates/lgwks-bot/src/ecs.rs:2620`); the `run_steps` pass then awaits those steps
on the caller's executor in exactly that order
(`crates/lgwks-bot/src/ecs.rs:3111`). It breaks on the first failure and records
...
`fire_plan` decides (`crates/lgwks-bot/src/ecs.rs:2406`), so selecting work by
...
- A condition whose `Evaluate<T>` implementation returns `Err` stops the chain
  with that error (`crates/lgwks-bot/src/ecs.rs:2609`). A structural failure in a
...
(`crates/lgwks-bot/src/ecs.rs:657`); `tick` takes it after the schedule runs, and

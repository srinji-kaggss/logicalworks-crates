...
(`crates/lgwks-bot/src/ecs.rs:3633`), because the condition on this executor *is*
...
(`crates/lgwks-bot/src/ecs.rs:3787`), so the effects fire in the order you wrote
the `.on` calls.
...
not cover (`crates/lgwks-bot/src/ecs.rs:4490`), and `Schedule::initialize` with
`ambiguity_detection: LogLevel::Error` rejects a schedule whose systems cannot be
totally ordered (`crates/lgwks-bot/src/ecs.rs:3360`).
...
(`crates/lgwks-bot/src/ecs.rs:3413`):
...
tick `observe_fold` treats it as changed (`crates/lgwks-bot/src/ecs.rs:2919`), so

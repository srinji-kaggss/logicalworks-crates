...
(`crates/lgwks-bot/src/ecs.rs:4630`), because the condition on this executor *is*
...
(`crates/lgwks-bot/src/ecs.rs:4011`), so the effects fire in the order you wrote
the `.on` calls.
...
totally ordered (`crates/lgwks-bot/src/ecs.rs:3634`).
...
(`crates/lgwks-bot/src/ecs.rs:3689`):
...
tick `observe_fold` treats it as changed (`crates/lgwks-bot/src/ecs.rs:3295`), so

...
(`crates/lgwks-bot/src/ecs.rs:4782`):
...
the world (`crates/lgwks-bot/src/ecs.rs:4799`):
...
- `EcsBot::assemble` admits every declared requirement before the world is built
  (`crates/lgwks-bot/src/ecs.rs:4782`), so a bot whose declared requirements are
...
that folds it into an abandoned entry (`crates/lgwks-bot/src/ecs.rs:3172`) is

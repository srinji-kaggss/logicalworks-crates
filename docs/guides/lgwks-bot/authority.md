...
(`crates/lgwks-bot/src/ecs.rs:4766`):
...
the world (`crates/lgwks-bot/src/ecs.rs:4783`):
...
- `EcsBot::assemble` admits every declared requirement before the world is built
  (`crates/lgwks-bot/src/ecs.rs:4766`), so a bot whose declared requirements are
...
that folds it into an abandoned entry (`crates/lgwks-bot/src/ecs.rs:3156`) is

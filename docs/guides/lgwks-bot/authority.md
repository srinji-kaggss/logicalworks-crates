...
(`crates/lgwks-bot/src/ecs.rs:4532`):
...
the world (`crates/lgwks-bot/src/ecs.rs:4549`):
...
- `EcsBot::assemble` admits every declared requirement before the world is built
  (`crates/lgwks-bot/src/ecs.rs:4533`), so a bot whose declared requirements are
...
that folds it into an abandoned entry (`crates/lgwks-bot/src/ecs.rs:2816`) is

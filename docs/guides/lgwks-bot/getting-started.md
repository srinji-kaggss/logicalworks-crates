...
(`crates/lgwks-bot/src/ecs.rs:727`), because the condition on this executor *is*
...
(`crates/lgwks-bot/src/ecs.rs:679`), so the effects fire in the order you wrote
the `.on` calls.
...
not cover (`crates/lgwks-bot/src/ecs.rs:837`), and `Schedule::initialize` with
`ambiguity_detection: LogLevel::Error` rejects a schedule whose systems cannot be
totally ordered (`crates/lgwks-bot/src/ecs.rs:878`).
...
(`crates/lgwks-bot/src/ecs.rs:456`):
...
tick `observe_fold` treats it as changed (`crates/lgwks-bot/src/ecs.rs:308`), so

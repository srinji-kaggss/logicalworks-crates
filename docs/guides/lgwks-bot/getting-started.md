...
(`crates/lgwks-bot/src/ecs.rs:3982`), because the condition on this executor *is*
...
(`crates/lgwks-bot/src/ecs.rs:3434`), so the effects fire in the order you wrote
the `.on` calls.
...
not cover (`crates/lgwks-bot/src/ecs.rs:4119`), and `Schedule::initialize` with
`ambiguity_detection: LogLevel::Error` rejects a schedule whose systems cannot be
totally ordered (`crates/lgwks-bot/src/ecs.rs:2947`).
...
(`crates/lgwks-bot/src/ecs.rs:3029`):
...
tick `observe_fold` treats it as changed (`crates/lgwks-bot/src/ecs.rs:3381`), so

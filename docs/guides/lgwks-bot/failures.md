...
synchronous adapter (`crates/lgwks-bot/src/ecs.rs:3598`) and `Bot::tick_async` is
the one to `await` from inside a runtime (`crates/lgwks-bot/src/ecs.rs:3487`).
...
(`crates/lgwks-bot/src/ecs.rs:2865`):
...
(`crates/lgwks-bot/src/ecs.rs:3268`); the `run_steps` pass then awaits those steps
on the caller's executor in exactly that order
(`crates/lgwks-bot/src/ecs.rs:3790`). It breaks on the first failure and records
...
`fire_plan` decides (`crates/lgwks-bot/src/ecs.rs:3045`), so selecting work by
...
- A condition whose `Evaluate<T>` implementation returns `Err` stops the chain
  with that error (`crates/lgwks-bot/src/ecs.rs:3275`). A structural failure in a
...
(`crates/lgwks-bot/src/ecs.rs:928`); `tick` takes it after the schedule runs, and

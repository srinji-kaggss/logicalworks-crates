...
synchronous adapter (`crates/lgwks-bot/src/ecs.rs:3362`) and `Bot::tick_async` is
the one to `await` from inside a runtime (`crates/lgwks-bot/src/ecs.rs:3251`).
...
(`crates/lgwks-bot/src/ecs.rs:2653`):
...
`fire_plan` decides (`crates/lgwks-bot/src/ecs.rs:2833`), so selecting work by
...
- A condition whose `Evaluate<T>` implementation returns `Err` stops the chain
  with that error (`crates/lgwks-bot/src/ecs.rs:3039`). A structural failure in a

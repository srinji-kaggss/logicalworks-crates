...
(`crates/lgwks-bot/src/ecs.rs:4387`), because the condition on this executor *is*
...
(`crates/lgwks-bot/src/ecs.rs:3790`), so the effects fire in the order you wrote
the `.on` calls.
...
not cover (`crates/lgwks-bot/src/ecs.rs:4526`), and `Schedule::initialize` with
`ambiguity_detection: LogLevel::Error` rejects a schedule whose systems cannot be
totally ordered (`crates/lgwks-bot/src/ecs.rs:3303`).
...
(`crates/lgwks-bot/src/ecs.rs:3385`):
...
tick `observe_fold` treats it as changed (`crates/lgwks-bot/src/ecs.rs:3737`), so

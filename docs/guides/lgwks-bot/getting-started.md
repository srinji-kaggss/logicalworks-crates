...
(`crates/lgwks-bot/src/ecs.rs:4314`), because the condition on this executor *is*
...
(`crates/lgwks-bot/src/ecs.rs:3730`), so the effects fire in the order you wrote
the `.on` calls.
...
not cover (`crates/lgwks-bot/src/ecs.rs:4453`), and `Schedule::initialize` with
`ambiguity_detection: LogLevel::Error` rejects a schedule whose systems cannot be
totally ordered (`crates/lgwks-bot/src/ecs.rs:3243`).
...
(`crates/lgwks-bot/src/ecs.rs:3325`):
...
tick `observe_fold` treats it as changed (`crates/lgwks-bot/src/ecs.rs:3651`), so

...
(`crates/lgwks-bot/src/ecs.rs:3786`), because the condition on this executor *is*
...
(`crates/lgwks-bot/src/ecs.rs:3290`), so the effects fire in the order you wrote
the `.on` calls.
...
not cover (`crates/lgwks-bot/src/ecs.rs:3923`), and `Schedule::initialize` with
`ambiguity_detection: LogLevel::Error` rejects a schedule whose systems cannot be
totally ordered (`crates/lgwks-bot/src/ecs.rs:2803`).
...
(`crates/lgwks-bot/src/ecs.rs:2885`):
...
tick `observe_fold` treats it as changed (`crates/lgwks-bot/src/ecs.rs:3237`), so

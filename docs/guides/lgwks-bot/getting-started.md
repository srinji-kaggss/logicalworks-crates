...
(`crates/lgwks-bot/src/ecs.rs:4394`), because the condition on this executor *is*
...
(`crates/lgwks-bot/src/ecs.rs:3797`), so the effects fire in the order you wrote
the `.on` calls.
...
not cover (`crates/lgwks-bot/src/ecs.rs:4533`), and `Schedule::initialize` with
`ambiguity_detection: LogLevel::Error` rejects a schedule whose systems cannot be
totally ordered (`crates/lgwks-bot/src/ecs.rs:3310`).
...
(`crates/lgwks-bot/src/ecs.rs:3392`):
...
tick `observe_fold` treats it as changed (`crates/lgwks-bot/src/ecs.rs:3718`), so

...
description: Director-selected Candle ownership, default-off feature boundary, transitive-surface limits and verification plan.
resource: /docs/candle-admission.md
tags: [dependencies, candle, ml, inference, tokenizers]
generated: {by: agent:opencode, at: 2026-09-18}
verified: {by: agent:opencode, method: explicit Director direction, policy source inspection and local gate execution}
status: published-parent-review-pending
stale_after: 2026-10-18
sources: [Director conversation 2026-09-18, /AGENTS.md, /docs/dependency-doctrine.md, /contract/APPROVED.toml, https://github.com/huggingface/candle, https://crates.io/crates/candle-transformers/0.11.0, https://crates.io/crates/tokenizers/0.23.2]
---

# Decision and actual authority

The Director directed embedding a System One-style decision model in Rust
applications and selected Candle as the inference runtime, with the explicit
instruction "Build inside the estate now". This is the actual human decision
recorded by `approved_by = "Director"` and `approved_on = "2026-09-18"`. This
document is agent-authored evidence of that direction, not a human-authored
document or cryptographic signature. The commit that adds the register blocks
and the authored edges is the signature, per `docs/dependency-doctrine.md`
§7 and the register header.
...
verifiable. Burn remains the estate-later option and `ort` the
...
# Transitive surface and egress — explicit limits
...
resulting tree is therefore network-capable. The estate runtime is intended to
load a local checkpoint and must not call the hub; that intent is a runtime
property to be verified, not implied by this admission. No advisory-clean or
universal safety claim is made for the closure.
...
4. Workspace fmt, clippy, and tests remain mandatory, and the package smoke is
   unchanged.
5. CPU compilation on the Linux CI lane is checked; the Metal backend is
   macOS-only and must be verified on a macOS job, not inferred.

# Nine-axis and lifecycle limits

The exact nine-axis invariants are those declared by the canonical assurance
contract; this admission predicts single ownership and opt-in compatibility and
nothing more. No frontier, hyperscale, portable-parity, or
fastest-correct-Rust claim follows from adopting a library. Fleet behaviour,
physical Ryzen profiles, human outcomes, production isolation, crash/recovery
acceptance, and end-to-end model parity remain unmeasured. Test processes are
finite, artifacts owned, and disposal Trash-only.

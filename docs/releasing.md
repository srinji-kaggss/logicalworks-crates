...
How a version reaches crates.io, and what has to be true first.

## 1. Publish order is forced, not chosen
...
authentication. Confirm credentials exist before starting, or the train stops
mid-chain:
...
CI runs these; run them locally too, and **run CI's commands rather than the
workspace ones**. `.cargo/config.toml` sets `rustc-workspace-wrapper =
"clippy-driver"`, so every `cargo check` here is a clippy run — but a
workspace-wide clippy does not compile every feature combination, and CI's
per-crate `--all-features` matrix reaches feature-gated code that a workspace
build skips. `Language::C` in `lgwks_ast` reached CI that way.
...
to the manifest without adding a bullet fails the lane.

## 3. Cut the release commit

Bump in one commit, all of it together — the four `[package] version` fields,
...
--workspace`). Then the `CHANGELOG.md` entry, per crate, with the behavioural
changes a consumer will actually hit — not only the signatures.

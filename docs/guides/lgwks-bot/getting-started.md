...
(`crates/lgwks-bot/src/ecs.rs:3720`), because the condition on this executor *is*
...
(`crates/lgwks-bot/src/ecs.rs:3224`), so the effects fire in the order you wrote
the `.on` calls.
...
not cover (`crates/lgwks-bot/src/ecs.rs:3857`), and `Schedule::initialize` with
`ambiguity_detection: LogLevel::Error` rejects a schedule whose systems cannot be
totally ordered (`crates/lgwks-bot/src/ecs.rs:2750`).
...
(`crates/lgwks-bot/src/ecs.rs:2832`):
...
tick `observe_fold` treats it as changed (`crates/lgwks-bot/src/ecs.rs:3172`), so

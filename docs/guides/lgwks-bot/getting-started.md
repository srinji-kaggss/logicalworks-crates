...
(`crates/lgwks-bot/src/ecs.rs:4335`), because the condition on this executor *is*
...
(`crates/lgwks-bot/src/ecs.rs:3738`), so the effects fire in the order you wrote
the `.on` calls.
...
not cover (`crates/lgwks-bot/src/ecs.rs:4474`), and `Schedule::initialize` with
`ambiguity_detection: LogLevel::Error` rejects a schedule whose systems cannot be
totally ordered (`crates/lgwks-bot/src/ecs.rs:3251`).
...
(`crates/lgwks-bot/src/ecs.rs:3333`):
...
tick `observe_fold` treats it as changed (`crates/lgwks-bot/src/ecs.rs:3685`), so
